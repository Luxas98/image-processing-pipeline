...
terraform apply -var="app=${APP_NAME}" -var="env=${CLIENT_NAME}" -var="project_id=${PROJECT_ID}" -var="region=${REGION}" -var="zone=${ZONE}" -var="cluster_name=${PRIMARY_CLUSTER_NAME}" -var="vault_address=${VAULT_ADDRESS}" -var="vault_ca=${VAULT_CA}" -var="vault_token=${VAULT_TOKEN}" -var="vault_backend_path=${VAULT_BACKEND_PATH}"
